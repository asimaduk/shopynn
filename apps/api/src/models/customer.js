 ...
 * `id` is `customers.id` or `customer_profiles.id` respectively.
 */
export const getAllCustomersService = async (user, requestQuery = {}) => {
    ...
    const [posResult, accResult] = await Promise.all([
        pool.query(posQuery, posParams),
        pool.query(accQuery, accParams),
    ]);

    const merged = [...posResult.rows, ...accResult.rows].sort(
...
export const createCustomerService = async (payload) => {
    // console.log('createCustomerService payload',payload);
...
export const updateCustomerService = async (payload, tenantId) => {
    console.log('updateCustomerService payload',payload);
