...
/** Read from user.company.settings or company object. */
...
export function isPriceMismatchError(err: any) {
	const code =
		err?.data?.data?.code ||
		err?.data?.code ||
...
export function getSaleApiErrorMessage(err: any, fallback = 'Could not complete sale.') {
	return (
		err?.data?.message ||
