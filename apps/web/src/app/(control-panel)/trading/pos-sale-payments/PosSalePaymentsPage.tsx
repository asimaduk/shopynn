...
import { useGetPosSalePaymentsQuery } from '../../billing/SubscriptionApi';
import PosSalePaymentsTable from './PosSalePaymentsTable';

export default function PosSalePaymentsPage() {
		...
		statusTab === 'all'
			...
			: statusTab === 'paid'
				...
				: statusTab === 'failed'
					? 'failed'
					: 'pending';
	const { data, isLoading, refetch, isFetching } = useGetPosSalePaymentsQuery({
		sale: query || undefined,
		status: statusQuery,
		sort_by: sortBy,
		sort_dir: sortDir
	});
	...
	const summary = useMemo(() => {
		const totalAmount = rows.reduce(
			(sum: number, row: any) => sum + Number(row?.face_amount ?? row?.amount ?? 0),
			0
		);
		const paid = rows.filter((row: any) =>
			['paid', 'success', 'completed'].includes(String(row?.status || '').toLowerCase())
		).length;
		const pending = rows.filter((row: any) =>
			['pending', 'otp', 'ongoing', 'send_otp'].includes(String(row?.status || '').toLowerCase())
		).length;
		return { count: rows.length, totalAmount, paid, pending };
	}, [rows]);
		...
		<Box className="flex h-full w-full flex-auto flex-col px-4 pb-8">
					...
					<div className="flex items-start gap-3">
						...
						<div className="min-w-0">
							...
							<Typography variant="body2" color="text.secondary" className="mt-1">
								Mobile money charges for counter sales — paid, pending, and failed.
							</Typography>
						</div>
					</div>
				</motion.span>
			...
			>
				...
				>
					<Typography variant="body2">
						<strong>Rows:</strong> {summary.count}
					</Typography>
					<Typography variant="body2">
					...
					<Typography variant="body2">
						...
						{summary.totalAmount.toLocaleString('en-GH', {
			...
			<Paper variant="outlined" className="rounded-xl overflow-hidden" sx={{ borderColor: 'divider' }}>
				{isLoading ? (
					...
					<Box className="p-12">
						<Typography color="text.secondary">No POS MoMo payments found.</Typography>
					</Box>
				) : (
					<PosSalePaymentsTable rows={rows} />
				)}
			</Paper>
		</Box>
	);
