'use client';

import Typography from '@mui/material/Typography';
import Button from '@mui/material/Button';
import { motion } from 'motion/react';
import FuseSvgIcon from '@fuse/core/FuseSvgIcon';
import PageBreadcrumb from 'src/components/PageBreadcrumb';
import Link from '@fuse/core/Link';

type SaleDetailHeaderProps = {
	date: string;
	formattedDate: string;
};

export default function SaleDetailHeader({ date, formattedDate }: SaleDetailHeaderProps) {
	return (
		<div className="flex grow-0 flex-1 w-full items-center justify-between space-y-2 sm:space-y-0 py-6 sm:py-8">
			<motion.span
				initial={{ x: -20 }}
				animate={{ x: 0, transition: { delay: 0.2 } }}
			>
				<div>
					<PageBreadcrumb className="mb-2" />
					<Typography className="text-4xl font-extrabold leading-none tracking-tight">
						Sale details — {formattedDate}
					</Typography>
					<Typography variant="body2" color="text.secondary" className="mt-1">
						Transactions and summary for this day
					</Typography>
				</div>
			</motion.span>
			<Button
				component={Link}
		to="/dashboards/analytics/daily-sales"
				variant="outlined"
				color="primary"
				startIcon={<FuseSvgIcon size={18}>heroicons-outline:arrow-left</FuseSvgIcon>}
			>
				Back to daily sales
			</Button>
		</div>
	);
}
