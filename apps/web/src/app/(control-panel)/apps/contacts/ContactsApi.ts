...
const COUNTRIES: Country[] = [
	{ id: 'gh', title: 'Ghana', iso: 'GH', code: '+233' },
	{ id: 'ng', title: 'Nigeria', iso: 'NG', code: '+234' },
	{ id: 'ke', title: 'Kenya', iso: 'KE', code: '+254' },
	{ id: 'za', title: 'South Africa', iso: 'ZA', code: '+27' },
	{ id: 'gb', title: 'United Kingdom', iso: 'GB', code: '+44' },
	{ id: 'us', title: 'United States', iso: 'US', code: '+1' }
];
const CONTACT_TAGS: Tag[] = [
	...
	.injectEndpoints({
		endpoints: (build) => ({
			...
			getContactsTag: build.query<GetContactsTagApiResponse, GetContactsTagApiArg>({
				queryFn: async (tagId) => {
					const tag = CONTACT_TAGS.find((t) => t.id === tagId);
					if (!tag) return { error: { status: 404, data: 'Tag not found' } as any };
					return { data: tag };
				},
				providesTags: ['contacts_tag']
			}),
			updateContactsTag: build.mutation<UpdateContactsTagApiResponse, UpdateContactsTagApiArg>({
				queryFn: async (tag) => ({ data: tag }),
