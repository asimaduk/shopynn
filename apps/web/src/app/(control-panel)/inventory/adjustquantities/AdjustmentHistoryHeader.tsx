...
type AdjustmentHistoryHeaderProps = {
	recordCount: number;
	productLinesTotal: number;
	isLoading?: boolean;
	dateRange: DateRange;
	onDateRangeChange: (range: DateRange) => void;
};

export default function AdjustmentHistoryHeader({
	recordCount,
	productLinesTotal,
	isLoading,
	dateRange,
	onDateRangeChange
}: AdjustmentHistoryHeaderProps) {
		...
		<div className="flex grow-0 flex-1 w-full items-center justify-between space-y-2 sm:space-y-0 py-6 sm:py-8">
			<motion.span initial={{ x: -20 }} animate={{ x: 0, transition: { delay: 0.2 } }}>
				<div>
					...
					<Typography className="text-4xl font-extrabold leading-none tracking-tight">Adjustment history</Typography>
					<Typography
						variant="body1"
					...
					>
						{isLoading ? (
							...
							<>
								...
								{productLinesTotal.toLocaleString()} item lines adjusted
							</>
						)}
					</Typography>
				</div>
			...
			<div className="flex flex-1 items-center justify-end gap-2">
				...
				>
					<Button
						...
						to="/inventory/adjustquantities/new"
						size={isMobile ? 'small' : 'medium'}
					>
						<FuseSvgIcon size={20}>heroicons-outline:plus</FuseSvgIcon>
						<span className="mx-1 sm:mx-2">Add new</span>
					</Button>
				</motion.div>
				<motion.div
					className="flex grow-0"
