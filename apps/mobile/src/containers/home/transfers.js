...
import TransferItem from './transfer_item';
...
// Parse date string like "Jan 4 @ 10:00am" to Date object
const parseDateString = (dateStr) => {
    ...
    // Parse time (e.g., "10:00am" or "2:30pm")
...
const ProductTransfers = ({ navigation }) => {
    const { colors } = useTheme();
    const user = useSelector(({ user }) => user);
    const subscriptionFeatures = useSelector(({ appSettings }) => appSettings?.subscriptionFeatures || []);
    const canCreate = canAccessScreen(user, 'NewTransfer', subscriptionFeatures);
    ...
    const loadTransfers = useCallback(async () => {
        try {
            ...
            const raw = await transfersApi.list(params);
    ...
    // Reload transfers whenever screen comes into focus or date range changes
    useFocusEffect(
        useCallback(() => {
            loadTransfers();
        }, [loadTransfers])
    ...
    const generateCSV = () => {
        const headers = 'ID,Source Store,Destination Store,Items Summary,Date,Status,Reason\n';
        const rows = filteredData.map((item) => {
            ...
            return [
                item.id || '',
                escapeCSV(item.source_warehouse_name),
                escapeCSV(item.destination_warehouse_name),
                escapeCSV(item.itemsSummary),
                escapeCSV(item.date),
...
<head>
    <meta charset="UTF-8">
    <title>Product Transfers Report</title>
    <style>
        body {
        ...
        setTimeout(async () => {
            try {
                ...
                if (format === 'csv' || format === 'excel') {
                    ...
                    const fileName = `Transfers_${dateRangeLabel}_${dateStr}.csv`;
                    ...
                    const fileName = `Transfers_${dateRangeLabel}_${dateStr}.html`;
        ...
        <SafeAreaView edges={['bottom', 'left', 'right']} style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            <AppModal
                ...
                <View style={localStyles.exportModalContent}>
                    <AppText 
                        label={`Export ${filteredData.length} transfer${filteredData.length !== 1 ? 's' : ''} as:`} 
                        fontSize={14} 
