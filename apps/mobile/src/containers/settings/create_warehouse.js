...
const CreateWarehouse = ({ navigation }) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (canCustomerSignupCodes && formData.reference_code?.trim()) {
    ...
    return (
        <KeyboardAvoidingView
            ...
            <SafeAreaView edges={['bottom', 'left', 'right']} style={{ flex: 1, backgroundColor: colors.background }}>
                <ScreenHeader onPress={backPress} label="Create Warehouse" />
                <View style={{ flex: 1 }}>
                    <ScrollView
                        ...
                        <View style={[styles.infoBanner, { backgroundColor: colors.primaryShade }]}>
                            ...
                            <AppText
                                label="Fill in the details to create a new warehouse or store location."
                                fontSize={13}
                    ...
                    <TouchableOpacity
                        activeOpacity={0.8}
                        onPress={handleSave}
                        disabled={isLoading || !formData.name.trim() || (formData.location_id == null && !formData.location?.trim())}
                        style={[styles.saveButton, (isLoading || !formData.name.trim() || (formData.location_id == null && !formData.location?.trim())) && styles.saveButtonDisabled]}>
                        {isLoading ? (
                            <ActivityIndicator color={colors.textInverse} />
                        ) : (
                            <>
                                <Lucide name="plus" color={colors.textInverse} size={18} style={{ marginRight: 8 }} />
                                <AppText label="Create Warehouse" variant={1} color={colors.textInverse} fontSize={16} />
                            </>
                        )}
                    </TouchableOpacity>
                </View>
                    ...
                    <ScrollView style={[styles.modalList, { backgroundColor: colors.surface }]} keyboardShouldPersistTaps="handled">
                        {locationOptions.map((opt) => (
                            <TouchableOpacity
                                ...
                                style={[styles.modalRow, { backgroundColor: formData.location === opt.name ? colors.primaryShade : colors.surface, borderBottomColor: colors.border }]}>
                                <Lucide name="map-pin" size={18} color={formData.location === opt.name ? config.THEME_COLOR : colors.textSecondary} />
                                <AppText label={opt.name} fontSize={16} style={{ flex: 1, marginLeft: 12 }} color={formData.location === opt.name ? config.THEME_COLOR : colors.text} />
