...
import CustomerPaymentItem from './customer_payment_item';
...
const CustomerPayments = ({ navigation, route }) => {
    ...
    const loadCustomerPayments = useCallback(async () => {
        ...
        try {
            ...
            const params = bounds
                ...
                : {};
            const raw = await paymentsApi.byCustomer(customerId, params);
            ...
            const mapped = (Array.isArray(list) ? list : []).map((p) => {
                const method =
                    ...
                    p.type ||
                    'Payment';
                const amount = String(p.amount ?? p.total_amount ?? p.value ?? 0);
                const status =
                    p.status ||
                    p.payment_status ||
                    p.state ||
                ...
                const reference =
                    ...
                    '';
                const dateSource = p.created_at || p.createdAt;
    ...
    }, [customerId, selectedDateRange, appliedCustomStartDate, appliedCustomEndDate]);
    ...
    const handleRefresh = useCallback(() => {
        setRefreshing(true);
        loadCustomerPayments().finally(() => setRefreshing(false));
    }, [loadCustomerPayments]);

    const generateCSV = () => {
        const headers = 'ID,Payment Method,Amount,Date,Status\n';
...
<body>
    <h1>${customerName}'s Payments Report</h1>
...
`;
        ...
        filteredData.forEach((item) => {
            const statusClass = item.status === 'Approved' || item.status === 'Cleared' ? 'status-approved' : '';
            html += `
            <tr>
                <td>${item.id || ''}</td>
                <td>${item.method || ''}</td>
        ...
        setTimeout(async () => {
            try {
                ...
                if (format === 'csv' || format === 'excel') {
                    ...
                    const fileName = `Customer_Payments_${dateRangeLabel}_${dateStr}.csv`;
                    ...
                    const fileName = `Customer_Payments_${dateRangeLabel}_${dateStr}.html`;
                ...
                <View style={localStyles.statsRow}>
                    <View style={[localStyles.statCard, localStyles.statCardCompact, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                        <View style={localStyles.statContent}>
                            <View style={{ flex: 1 }}>
                                <AppText label="Payments" fontSize={11} color={colors.textSecondary} style={localStyles.statLabel} />
                                <AppText
            ...
            <FlashList
                ...
                renderItem={({ item, index }) => (
                    <CustomerPaymentItem
                        item={item}
                        index={index}
                        onPress={() => navigation.navigate('CustomerPaymentDetails', { item })}
                    />
                )}
                ...
                ListEmptyComponent={() => (
                    <View style={{ alignItems: 'center', marginTop: 50 }}>
                        <Lucide name="credit-card" color={colors.textTertiary} size={48} />
                        <AppText label="No payments found" color={colors.textSecondary} style={{ marginTop: 12 }} />
