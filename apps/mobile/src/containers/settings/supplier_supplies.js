...
import SupplierProductItem from './supplier_product_item';
...
const SupplierSupplies = ({ navigation, route }) => {
    const { colors } = useTheme();
    const { supplierId, supplierName } = route.params || { supplierName: 'Supplier' };
    ...
    const filteredData = useMemo(() => {
        ...
        if (searchText.trim()) {
            ...
            result = result.filter((item) => {
                const name = (item.invoice_number || '').toLowerCase();
                const amount = String(item.total_amount || '');
                const qty = String(item.number_of_items || '');
                return (
                    name.includes(q) ||
                    amount.includes(q) ||
                    qty.includes(q)
    ...
    const totalSupplyAmount = useMemo(() => {
        return filteredData.reduce((sum, row) => sum + Number(row.total_amount ?? 0), 0);
    }, [filteredData]);
    ...
    const loadSupplierSupplies = useCallback(async () => {
        ...
        try {
            ...
            const params = bounds
                ...
                : {};
            const raw = await suppliersApi.getPurchases(supplierId, params);
            console.log('raw', raw);
            ...
            //         p.name ||
            //         'Product';
            //     const quantity = p.quantity || p.qty || p.units || 0;
            //     const amount = String(
            //         p.total_amount ??
            //             p.amount ??
            //             p.line_total ??
            ...
            //             0,
            //     );
            //     const dateSource = p.date || p.created_at || p.createdAt;
...
<head>
    <meta charset="UTF-8">
    <title>Supplier Supplies Report</title>
    <style>
        body {
...
<body>
    <h1>${supplierName}'s Supplies Report</h1>
    <div class="header-info">
        <p><strong>Date Range:</strong> ${dateRangeLabel}</p>
        <p><strong>Export Date:</strong> ${exportDate}</p>
        <p><strong>Total Supplies:</strong> ${filteredData.length}</p>
...
`;
        ...
        setTimeout(async () => {
            try {
                ...
                if (format === 'csv' || format === 'excel') {
                    ...
                    const fileName = `Supplier_Supplies_${dateRangeLabel}_${dateStr}.csv`;
                    ...
                    const fileName = `Supplier_Supplies_${dateRangeLabel}_${dateStr}.html`;
    ...
    if (loading) {
        return (
            <SafeAreaView edges={['bottom', 'left', 'right']} style={[styles.container, { backgroundColor: colors.background, justifyContent: 'center', alignItems: 'center' }]}>
                ...
                <AppText label="Loading supplier supplies..." fontSize={14} color={colors.textSecondary} style={{ marginTop: 12 }} />
            </SafeAreaView>
        );
    }
    ...
    return (
        <SafeAreaView edges={['bottom', 'left', 'right']} style={[styles.container, { backgroundColor: colors.background }]}>
            <ScreenHeader onPress={backPress} label={`Supplies | ${supplierName}`}>
                ...
                <View style={localStyles.statsRow}>
                    <View style={[localStyles.statCard, localStyles.statCardCompact, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                        <View style={localStyles.statContent}>
                            <View style={{ flex: 1 }}>
                                <AppText label="Supplies" fontSize={11} color={colors.textSecondary} style={localStyles.statLabel} />
                                <AppText
            ...
            {filteredData.length > 0 && (
                <View
                    style={[
                        localStyles.summaryStrip,
                        { backgroundColor: colors.surface, borderColor: colors.border },
                    ]}>
                    <View style={[localStyles.summaryStripIcon, { backgroundColor: colors.primaryShade }]}>
                        <Lucide name="wallet" color={config.THEME_COLOR} size={18} />
                    </View>
                    <View style={{ flex: 1 }}>
                        <AppText label="Total amount" fontSize={11} color={colors.textTertiary} />
                        <AppText
                            label={formatCurrency(totalSupplyAmount)}
                            fontSize={17}
                            variant={1}
                    ...
                    <View style={{ alignItems: 'flex-end' }}>
                        <AppText label="Purchases" fontSize={11} color={colors.textTertiary} />
                        <AppText
                            label={`${filteredData.length}`}
                            fontSize={17}
                            variant={1}
            ...
            <FlashList
                ...
                ListEmptyComponent={() => (
                    <View style={{ alignItems: 'center', marginTop: 50 }}>
                        ...
                        <AppText label="No supplies found" color={colors.textSecondary} style={{ marginTop: 12 }} />
            ...
            <AppModal
                ...
                <View style={[localStyles.exportModalContent, { backgroundColor: colors.surface }]}>
                    <AppText
                        label={`Export ${filteredData.length} suppl${filteredData.length !== 1 ? 'ies' : 'y'} as:`}
                        fontSize={14}
