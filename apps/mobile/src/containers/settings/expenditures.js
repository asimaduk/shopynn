...
import ExpenditureItem from './expenditure_item';
    ...
    const loadExpenditures = useCallback(async () => {
        try {
            ...
            const params = bounds
                ...
                : {};
            const raw = await expensesApi.list(params);
        ...
        if (searchText.trim()) {
            ...
            result = result.filter(
                (item) =>
                    ...
                    item.creator_first_name?.toLowerCase().includes(q) ||
                    item.creator_last_name?.toLowerCase().includes(q) ||
                    item.payment_method?.toLowerCase().includes(q) ||
                    item.amount?.includes(q) ||
                    item.expense_date?.toLowerCase().includes(q) ||
                    item.note?.toLowerCase().includes(q)
            );
        }

        return result;
    }, [expenditures, searchText, selectedDateRange, customStartDate, customEndDate]);
        ...
        <SafeAreaView edges={['bottom', 'left', 'right']} style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            <AppModal
                ...
                <View style={[localStyles.exportModalContent, { backgroundColor: colors.surface }]}>
                    <AppText
                        label={`Export ${filteredData.length} expenditure${filteredData.length !== 1 ? 's' : ''} as:`}
                        fontSize={14}
